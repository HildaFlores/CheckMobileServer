...
//vehInsert(object);
    ...
    public static int executeUpdate(Object object, ObjetosDB dbObject) {
        int registrosActualizados = 0;

        switch (dbObject) {

            case INSPECCION_VEHICULO: {
                registrosActualizados = llamarFunctionActualizaInspeccion(object);
                break;
            }
            case PEDIDO_ENC: {
                registrosActualizados = llamarFunctionActualizarOrden(object);
            }
    ...
    public static int executeUpdate(String jsonObject, ObjetosDB objetosDB) {
        ...
        switch (objetosDB) {
            ...
            case PEDIDO_ENC: {
                registrosActualizados = anularOrdenTrabajo(jsonObject);
                break;
            }
        }
        return registrosActualizados;
    ...
    private static List<Object> getPedido_producto(ResultSet resultSet) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            dato.setDescripcion_producto(resultSet.getString(PEDIDO_PRODUCTO.DESCRIPCION_SERVICIO));
    ...
    private static int pedidoInsert(Object object) {
        ...
        try (Connection conexion = getThinConnection();
            ...
            for (PedidoEnc dato : pedido) {
                ...
                preparedStatement.setString(18, dato.getFechaDocumento());
    ...
    private static int anularOrdenTrabajo(String object) {
        int insertedRows = 0;
        try (Connection conexion = getThinConnection();
                CallableStatement cst = conexion.prepareCall("{? = call PKG_ORDENES_TRABAJO.anular_orden_trabajo(?,?,?)}");) {

            conexion.setAutoCommit(false);
            cst.registerOutParameter(1, Types.INTEGER);
            cst.setString(2, Constantes.ID_EMPRESA);
            cst.setString(3, object);
            cst.setString(4, "OTT");
            cst.execute();
            conexion.commit();
            insertedRows = cst.getInt(1);
    ...
    private static int llamarFuncionInsertOrdenTrabajoDet(Object object) {
        ...
        try (Connection conexion = getThinConnection();
            ...
            for (PedidoProducto enc : pedido) {
                ...
                cst.setString(5, enc.getDescripcion_producto());
                cst.setString(6, enc.getId_impuesto());
                cst.setString(7, enc.getPrecio());
                cst.setString(8, enc.getCantidad());
                cst.setString(9, enc.getUnidad());
                cst.setString(10, enc.getPorc_descuento());
                cst.setString(11, enc.getMonto_impuesto());
                cst.setString(12, enc.getMonto_descuento());
                cst.setString(13, enc.getImporte_itbis());
                cst.setString(14, enc.getPrecio_neto());
                cst.setString(15, enc.getObservaciones());
                cst.setString(16, enc.getPrecio_bruto());
                cst.setString(17, enc.getDescuento());
                cst.setString(18, enc.getPrecio_final_sin_impuestos());
                cst.setString(19, enc.getPorc_impuesto());
                cst.setString(20, enc.getPrecio_final_con_impuestos());
                cst.setString(21, enc.getImporte());
                cst.addBatch();
            }
            insertedRows = cst.executeBatch();
            conexion.commit();
    ...
    private static int llamarFunctionActualizaInspeccion(Object object) {

        int updatedRows = 0;

        ArrayList<InspeccionVehiculo> inspeccion = (ArrayList<InspeccionVehiculo>) object;
        try (Connection conexion = getThinConnection();
                CallableStatement cst = conexion.prepareCall("{? = call PKG_INSPECCION.actualizar_inspeccion(?,?,"
                        + "?,?,?,?,?,?)}");) {
            ...
            for (InspeccionVehiculo ins : inspeccion) {
                cst.setString(2, Constantes.ID_EMPRESA);
                cst.setString(3, "IV");
                cst.setString(4, ins.getSerieGomas());
                cst.setString(5, ins.getNivelCombustible());
                cst.setString(6, ins.getObservaciones());
                cst.setString(7, ins.getKilometraje());
                cst.setString(8, ins.getMotor());
                cst.setString(9, ins.getId());
            }
            cst.execute();
            conexion.commit();
            updatedRows = cst.getInt(1);
    ...
    private static int llamarFunctionActualizarOrden(Object object) {
        int updatedRows = 0;

        ArrayList<PedidoEnc> inspeccion = (ArrayList<PedidoEnc>) object;
        try (Connection conexion = getThinConnection();
                CallableStatement cst = conexion.prepareCall("{? = call PKG_ORDENES_TRABAJO.actualizar_orden_trabajo(?,?,"
                        + "?,?,?,?,?,?)}");) {

            conexion.setAutoCommit(false);
            cst.registerOutParameter(1, Types.INTEGER);
            for (PedidoEnc ins : inspeccion) {
                cst.setString(2, ins.getId());
                cst.setString(3, "OTT");
                cst.setString(4, Constantes.ID_EMPRESA);
                cst.setString(5, ins.getFechaPedido());
                cst.setString(6, ins.getPermite_pieza_reemplazo());
